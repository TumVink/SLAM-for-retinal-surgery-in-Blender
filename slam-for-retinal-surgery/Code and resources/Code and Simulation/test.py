...
def create_slide_movement(points=4): #100 
    global p_insert
    positions=list()
    x=np.zeros(points)
    y=np.zeros(points)
...
def contour_to_position(contour):
    ...
    return x_1,y_1,z_1,x_2,y_2,z_2,ellipse_center_x,ellipse_center_y,ellipse_center_z
...
def test_scenario(spotlight1,spotlight2,spotlight3,x=0,y=0,z=5,angle_1=0,angle_2=0,angle_3=0,img=0,single=True):
    global p_insert
    #output location
    file="/home/jingsong/blender_file/Master Thesis-EyeSpotlight/Code and Simulation/temp/tmp"+str(img)+".png"
    ...
    for contour in contours:
        if len(contour)>5:
            x1,y1,z1,x2,y2,z2,xc,yc,zc=contour_to_position(contour)
    ...
    if single:
        return avg_x,avg_y,avg_z,dist*1000,[],dist2proj
